# A Two Channel 30 MHz Data Logging Frequency Counter For The Teensy 4.0
This is a two channel 30 MHz data logging frequency counter based on the Teensy 4.0 Development Board.  Data is transmitted directly via serial port so that it can be recorded and later analyzed via spreadsheet.  There is no external display to show the measured frequency directly.  You must use a serial terminal program to view the data.   I use CoolTerm which offers a means to save the received data to a file.  Alternatively the data text can be copied directly from the screen of the serial terminal.  CoolTerm is available for MacOS, Windows, Linux and RaspberryPi. 
