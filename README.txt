...
The reference clock is rock solid and the reported ch1 frequency error and drift are due to be primarily from the Teensy cpu clock.  (This is the assumed to be the most probable reason as we assume that the 10 MHz reference frequency source has a better and more stable oscillator.)
The Teensy clock is rock solid and the frequency error and drift are due to the reference oscillator.  (Probably not.)
The minimal long term frequency drift of ch1 is the result of nearly equal and opposte drift rates for the reference clock and the Teensy cpu clock that cancel each other out.  This is safely assumed to be not the case.  
